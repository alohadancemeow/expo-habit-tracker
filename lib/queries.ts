...
export const useTodayCompletions = (userId: string) => {
    return useQuery({
        ...
        queryFn: async () => {
            ...
            return response.documents as unknown as HabitCompletion[];
        },
        enabled: !!userId,
        staleTime: 1000 * 60, // Consider data fresh for 1 minute
        refetchInterval: 1000 * 30, // Refetch every 30 seconds
...
export const useDeleteHabit = () => {
    ...
    return useMutation({
        mutationFn: async (habitId: string) => {
            try {
                const completedHabitToDelete = await databases.listDocuments(
                    DATABASE_ID,
                    COMPLETIONS_COLLECTION_ID,
                ...
                await databases.deleteDocument(
                    DATABASE_ID,
                    COMPLETIONS_COLLECTION_ID,
                    completedHabitToDelete.documents[0].$id
                );
            } catch (error) {
                console.log(error);
            }
